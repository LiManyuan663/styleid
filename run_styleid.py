...
def main():
    ...
    # parser.add_argument('--ckpt', type=str, default='/root/autodl-tmp/huggingface/hub/models--CompVis--stable-diffusion-v-1-4-original/snapshots/f0bb45b49990512c454cf2c5670b0952ef2f9c71/sd-v1-4.ckpt', help='model checkpoint')
        ...
        for block_idx, block in enumerate(blocks):
            if len(block) > 1 and "SpatialTransformer" in str(type(block[1])):
                if block_idx in self_attn_output_block_indices:
                    # self-attn
                    # print(block[1].transformer_blocks[0].attn1)
                    # v2.1
    ...
    begin = time.time()
    for index_sty,sty_name in enumerate(sty_img_list):
        print("========第"+str(index_sty)+"风格=======")
        ...
        for index_cnt,cnt_name in enumerate(cnt_img_list):
            print("========第"+str(index_cnt)+"内容=======")
